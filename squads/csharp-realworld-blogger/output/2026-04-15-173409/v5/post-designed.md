...
- Status: approved
- Ajustes aplicados:
  - estrutura de seções reforçada para separar sintoma, decisão e implementação;
  - tabela adicionada para comparar opções arquiteturais e risco operacional;
  - bloco de código mantido para explicitar deduplicação e threshold em C#/.NET;
  - diagrama Mermaid adicionado para reduzir carga cognitiva no fluxo entre aplicação, CloudWatch e Lambda.
