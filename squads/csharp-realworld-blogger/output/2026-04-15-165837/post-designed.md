...
- Status: approved
- Ajustes aplicados:
  - estrutura editorial reorganizada para melhorar progressão causal;
  - tabela adicionada para comparar guardrails e falhas evitadas;
  - bloco de código mantido para ilustrar confidence gate e validação de evidência;
  - diagrama não incluído porque tabela e snippet já explicam o fluxo sem aumentar carga cognitiva.
...
Todo time que opera serviços .NET em produção conhece a cena: um incidente começa, o volume de alerta sobe e, antes de qualquer correção útil, o canal já está cheio de prints, hipótese e ruído. O custo não está só na falha. Está no tempo até a primeira leitura operacional confiável.
...
| Evidence IDs obrigatórios | Alucinação sem lastro observável | Necessidade de montar pacote de evidências melhor |
...
No pipeline em C#/.NET, a resposta do modelo foi tratada como objeto restrito, não como texto livre confiável por padrão.
